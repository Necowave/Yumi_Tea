...
# Состояния разговора
(CHOOSE_LANGUAGE, MAIN_MENU, ORDER_CATEGORY, CHOOSE_FLAVOR, CHOOSE_TOPPINGS, 
 CONFIRM_ORDER, ADD_LOCATION, PAYMENT, SETTINGS, CONTACT_INPUT) = range(10)
...
async def settings(update: Update, context):
    ...
    await query.edit_message_text("Выбери язык:" if get_user(user_id)[1] == 'ru' else "Choose your language:", reply_markup=reply_markup)
...
async def set_language(update: Update, context):
    query = update.callback_query
    await query.answer()
    lang = query.data.split('_')[2]  # например, 'set_lang_ru' -> 'ru'
    user_id = query.from_user.id
...
async def confirm_order(update: Update, context):
    ...
    missing_fields = [field for field in ['category', 'flavor', 'toppings'] if not order_data.get(field)]
    if missing_fields:
    ...
    try:
        order_id, price = create_order(user_id, order_data['category'], order_data['flavor'], order_data['toppings'])
...
async def add_location(update: Update, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text(get_text(user_id, 'payment', price=price), reply_markup=reply_markup)
...
async def handle_location(update: Update, context):
    ...
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(get_text(user_id, 'payment', price=price), reply_markup=reply_markup)
        context.user_data['state'] = 'PAYMENT'
        return PAYMENT
...
async def test_payment(update: Update, context):
    ...
    if 'order_id' not in context.user_data:
        await query.edit_message_text(get_text(user_id, 'order_not_found'))
        return ConversationHandler.END
    await query.edit_message_text("Тестовая оплата прошла успешно!" if get_user(user_id)[1] == 'ru' else "Test payment completed successfully!")
...
async def view_menu(update: Update, context):
    """Обработчик кнопки '📖 Посмотреть меню' в главном меню."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    keyboard = []
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Regex(r'^(🏠 Главн(ое)? меню|🏠 Main Menu)$'), back_to_main_menu_text))
